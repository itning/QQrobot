package top.itning.webqq.client;
...
public class SmartQQClient implements Closeable {
    ...
    private void getVfwebqq() {
        ...
        while (response.getStatusCode() == NOT_FOUNT_CODE && retryTimes4Vfwebqq > 0) {
    ...
    public List<Group> getGroupList() {
        ...
        while (response.getStatusCode() == NOT_FOUNT_CODE && retryTimes4getGroupList > 0) {
    ...
    public UserInfo getAccountInfo() {
        ...
        while (response.getStatusCode() == NOT_FOUNT_CODE && retryTimes4AccountInfo > 0) {
    ...
    private Response<String> postWithRetry(ApiURL url, JSONObject r) {
        ...
        do {
            response = post(url, r);
            times++;
        } while (times < RETRY_TIMES && response.getStatusCode() != SUCCESS_CODE);
        return response;
    }
    ...
    private static JSONObject getResponseJson(Response<String> response) {
        ...
        if (retCode == null) {
            throw new RequestException("请求失败，Api返回异常 retCode == null");
        } else if (retCode != 0) {
            switch (retCode) {
                case 103: {
    ...
    private static String hash(long x, String k) {
        ...
        for (long aU1 : u1) {
            v1.append(n1[(int) ((aU1 >> 4) & 15)]);
            v1.append(n1[(int) (aU1 & 15)]);
        }
        return v1.toString();
    }
